...
if [[ ${CB_PRODUCTION_BUILD} == 'true' ]]; then
    sign_flags="--force --timestamp --verbose  --options runtime --preserve-metadata=identifier,entitlements,requirements --entitlements ${WORKSPACE}/build/scripts/codesign/entitlements.xml"
    echo ------- Unlocking keychain -----------
    set +x
    security unlock-keychain -p `cat ~/.ssh/security-password.txt` ${HOME}/Library/Keychains/login.keychain
    set -x
